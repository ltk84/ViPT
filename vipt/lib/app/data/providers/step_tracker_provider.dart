...
class StepTrackerProvider implements SqfliteHelper<int, StepTracker> {
  @override
  ...
  @override
  Future<StepTracker?> fetch(int id) async {
    final db = await DatabaseProvider.database;
    final List<Map<String, dynamic>> maps =
        await db!.query(tableName, where: 'id = ?', whereArgs: [id]);
    if (maps.isEmpty) return null;
    return StepTracker.fromMap(maps[0]);
  }

  @override
  Future<List<StepTracker>> fetchAll() async {
    final db = await DatabaseProvider.database;
    final List<Map<String, dynamic>> maps = await db!.query(tableName);

    return List.generate(
        maps.length, (index) => StepTracker.fromMap(maps[index]));
  }

  @override
  Future<List<StepTracker>> fetchByDate(DateTime dateTime) async {
    final db = await DatabaseProvider.database;
    final String begin =
  ...
  @override
  Future<void> update(int id, StepTracker obj) async {
    final db = await DatabaseProvider.database;
